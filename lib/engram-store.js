...
export function searchObservations(store, { query, project, limit = 5, includeShared = true } = {}) {
  ...
  try {
    return store
      .prepare(
        `SELECT o.id, o.title, o.content, o.project, o.scope, o.type, o.pinned, o.updated_at,
...
export function formatInjection(rows, { budget = 1200, topK = 2, header = 'Память проекта (engram), возможно релевантное:' } = {}) {
  ...
  for (const row of rows.slice(0, topK)) {
    ...
    const line = `- [${row.id}${shared ? '·общее' : ''}] ${row.title}${row.content ? ` — ${firstLine(row.content)}` : ''}`
