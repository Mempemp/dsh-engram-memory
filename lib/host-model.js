 ...
 *
 * Ключей здесь нет и не должно быть: маршрут (провайдер + модель) задаёт человек
 * в настройках, а креденшелы разрешает зарегистрированный адаптер DSH. Разговор
...
export function createHostModel(ctx, { defaultTimeoutMs = 90000, log = () => {} } = {}) {
  ...
  return async function complete(messages, { timeoutMs = defaultTimeoutMs, signal, route } = {}) {
    if (route === undefined) throw new Error('модель для обработки не выбрана: укажите её в настройках DSH')
    const llm = ctx.llm
