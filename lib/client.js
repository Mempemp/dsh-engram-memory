...
/* Поля с подписями: рядом с кнопкой они читались как «непонятно что». */
.pem-fields { display: flex; flex-wrap: wrap; align-items: center; gap: 16px; }
.pem-field { display: inline-flex; align-items: center; gap: 8px; }
.pem-field__label { font-size: 12px; line-height: 1.5; opacity: .72; white-space: nowrap; }
...
/* Закрытый список прячем правилом, а не одним атрибутом hidden: у класса выше
   стоит display: flex, и он перебивает системное [hidden] — список выпадал сам. */
.pem-drop__list[hidden] { display: none; }
    ...
    function Section() {
      ...
      const load = useCallback(async () => {
        try {
          ...
          setState(data)
          // Обнулённый проект из очереди уходит: если фильтр указывает на него,
          // снимаем фильтр — иначе в поле остаётся пустота, а следующий запуск
          // искал бы заметки там, где их уже нет.
          setChoice((value) => (value !== '' && !(data.projects ?? []).some((bucket) => bucket.project === value) ? '' : value))
          setError(null)
        } catch (problem) {
      ...
      const run = async () => {
        ...
        try {
          ...
          // После прохода фильтр возвращается к «все проекты»: выбранный проект
          // мог опустеть, и пустое поле читалось как поломка вкладки.
          setChoice('')
          await load()
        } catch (problem) {
          setError(problem.message)
      ...
      // Значение выбора — без пробелов вокруг слэша: так оно совпадает с ключом
      // пункта списка. Иначе подпись кнопки оказывалась пустой.
      const currentModelValue = state.model === null || state.model === undefined ? '' : `${state.model.provider}/${state.model.model}`
      ...
      return jsxs('div', { className: 'pem-root', children: [
        ...
        jsxs(Card, {
          ...
          children: [
            ...
            jsxs('div', { className: 'pem-fields', children: [
              jsxs('span', { className: 'pem-field', children: [
                jsx('span', { className: 'pem-field__label', children: 'Проект' }),
                jsx(Dropdown, {
                  items: projectItems,
                  value: choice,
                  placeholder: `все проекты · ${unprocessed}`,
                  disabled: processing,
              ...
              jsxs('span', { className: 'pem-field', children: [
                jsx('span', { className: 'pem-field__label', children: 'Модель' }),
                modelItems.length === 0
                  ? jsx('span', { className: 'pem-chip', children: currentModel === '' ? 'не выбрана' : currentModel })
                  : jsx(Dropdown, {
                      items: modelItems,
                      value: currentModelValue,
                      placeholder: currentModel === '' ? 'не выбрана' : currentModel,
                      disabled: processing || busy,
                      title: 'Модель, которой сводятся заметки',
                      onPick: pickModel
                    })
              ] })
            ] }),
        ...
        jsxs(Card, {
          title: 'Память в диалогах',
          children: [
            jsxs('div', { className: 'pem-row', key: 'server', children: [
              jsx('span', { children: 'Модель ищет и дописывает сама' }),
              jsx('span', {
                className: mcp.declared ? 'pem-ok' : 'pem-warn',
                children: mcp.declared === false
                  ? 'не подключено'
                  : (mcp.needsRestart ? 'заработает после перезапуска' : 'подключено')
              })
            ] }),
            jsxs('div', { className: 'pem-row', key: 'binary', children: [
              jsx('span', { children: 'Версия Engram' }),
              jsx('span', { children: mcp.needsRestart
                ? `${mcp.harnessVersion ?? '—'} → ${mcp.packageVersion ?? '—'} (сменится после перезапуска)`
                : (mcp.harnessVersion ?? mcp.packageVersion ?? '—') })
            ] }),
            mcp.declared === false
              ? jsx('p', { className: 'pem-warn', children: 'Пока подключения нет, модель не сможет ни искать, ни дописывать память. Заметки о работе сохраняются сами.' })
              : null
          ]
        }),
