...
.pem-select { height: 32px; padding: 0 8px; border-radius: 8px; border: 1px solid var(--dsw-alias-border-l2, rgba(127,127,127,.35)); background: transparent; color: inherit; font-family: var(--dsw-font-family, inherit); font-size: 13px; cursor: pointer; }
.pem-select:disabled { cursor: not-allowed; opacity: .6; }
.pem-ok { color: #4caf7d; }
...
`
    ...
    /**
     * Цена нажатия до того, как его нажали: сколько будет проходов и сколько
     * токенов уйдёт в модель. Формулировки без падежей — числа бывают любые.
     */
    const estimateText = (estimate) => {
      ...
      const tail =
        estimate.capped === true
          ? ` За одно нажатие больше ${estimate.maxPasses} проходов не делается — остаток обрабатывается следующим нажатием.`
          : ''
      return `Будет проходов: ${Number(estimate.passes) || 0}, за проход до ${Number(estimate.perPassNotes) || 0} заметок, ≈${number(estimate.tokens)} токенов на входе (оценка по ${Number(estimate.charsPerToken) || 3} знака на токен).${tail}`
    }
    ...
    function Section() {
      ...
      return jsxs('div', { className: 'pem-root', children: [
        ...
        jsxs(Card, {
          title: 'Проекты',
          headExtra: jsx('span', { className: 'pem-muted', children: projects.length === 0 ? 'очередь пуста' : `в очереди: ${projects.length}` }),
          children: [
            projects.length === 0
              ? jsx('p', { className: 'pem-muted', children: 'Несведённых заметок нет — обрабатывать нечего.' })
              : jsx('div', { children: projects.map((bucket) =>
                  jsxs('div', { className: 'pem-row', key: bucket.project, children: [
                    jsx('span', { children: bucket.project }),
                    jsx('span', { className: 'pem-muted', children: `${bucket.unprocessed} из ${bucket.total} ещё не сведены` })
                  ] })
                ) }),
            ...
            jsx('p', { className: 'pem-muted', children: 'Проекты берутся из базы, а не из открытых окон: видно и то, что накопилось в закрытых. Модель — из настроек DSH («Модели»), собственных ключей у плагина нет.' }),
            state.workspace
              ? jsx('p', { className: 'pem-muted', children: `Последний рабочий каталог: ${state.workspace}` })
              : null
          ]
        }),
        jsxs(Card, {
          ...
          children: [
            ...
            jsx('div', { className: 'pem-actions', children: [
              ...
              jsx('select', {
                ...
                value: choice,
                disabled: processing,
                title: 'Ограничить проход одним проектом — необязательно',
                onChange: (event) => setChoice(event.target.value),
                children: [
                  jsx('option', { value: '', key: 'all', children: 'все проекты' }),
                  ...projects.map((bucket) => jsx('option', { value: bucket.project, key: bucket.project, children: bucket.project }))
                ]
              }),
              processing
        ...
        jsxs(Card, {
          ...
          children: [
            mcp.declared === false
              ? jsxs('div', { className: 'pem-row', key: 'mcp-warning', children: [
                  jsx('span', { className: 'pem-warn', children: 'MCP-сервер не объявлен' }),
                  jsx('span', { className: 'pem-muted', children: 'общий слой наполнять нечем' })
                ] })
              : null,
            ...connectionRows,
            jsx('p', { className: 'pem-muted', children: 'MCP обязателен: им модель ищет и пишет саму память, в том числе в общий слой, — автосохранение ходов пишет только в проект. Без сервера работают правила, подача и автосохранение в проект, а общий слой остаётся пустым.' }),
            jsx('p', { className: 'pem-muted', children: 'То же действие доступно командой /memory-consolidate: без аргумента — все проекты, с именем проекта — только он, «черновик» — показать вывод, ничего не записывая.' })
          ]
        }),
