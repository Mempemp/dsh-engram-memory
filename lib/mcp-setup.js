/**
 * Самодостаточность пакета: MCP-половина (engram и прокладка) лежит внутри
...
/**
 * Снимок «где сейчас работаем». Его пишет хост-половина на каждом ходу, а читает
 * прокладка MCP при запуске. Без него сервер памяти считает проектом свой
 * рабочий каталог — каталог запуска десктопа, — и записи модели уходят в
 * `launch-root` вместо рабочей области.
 */
export function currentProjectFile(env = process.env) {
  const base = typeof env.LOCALAPPDATA === 'string' && env.LOCALAPPDATA.trim() !== '' ? env.LOCALAPPDATA.trim() : null
  if (base === null) return null
  return join(base, 'DSH-1C', 'engram-current-project.json')
}

/** Записать снимок. Пустое имя проекта — не снимок: файл не трогаем. */
export function writeCurrentProject(project, workspace, env = process.env) {
  const path = currentProjectFile(env)
  if (path === null) return null
  if (typeof project !== 'string' || project.trim() === '') return null
  try {
    mkdirSync(dirname(path), { recursive: true })
    writeFileSync(path, `${JSON.stringify({ project: project.trim(), workspace: typeof workspace === 'string' ? workspace : '', at: new Date().toISOString() }, null, 2)}\n`, 'utf8')
    return path
  } catch {
    return null
  }
...
/** Прочитать снимок: нет файла или он битый — проекта нет. */
export function readCurrentProject(env = process.env) {
  const path = currentProjectFile(env)
...
/** Версия MCP-половины: файл-маркер рядом с engram.exe. */
...
/**
 * Узел для запуска прокладки. Десктоп запускает MCP-сервер сам, поэтому берём узел из
 * его же установки: свой процесс может быть самим приложением, из которого
...
/**
 * Раскладывает engram и прокладку в `<harness>/mcp/engram/package`. Версия помечена
 * файлом VERSION: совпала — копирования нет.
 *
 * Запертый engram.exe (сервер уже запущен и держит файл) — не ошибка: копия
...
export function ensureMcpPackage(harness, bundled, { log = () => {} } = {}) {
  ...
  if (existsSync(join(target, 'engram.exe')) && mcpVersion(target) === wanted && wanted !== '' && sameText(join(target, SHIM), join(bundled, SHIM))) {
...
/**
 * Одинаковое содержимое. Прокладка меняется чаще engram.exe, поэтому её копию
 * сверяем по содержимому: иначе правка прокладки не доехала бы до харнесса.
 */
