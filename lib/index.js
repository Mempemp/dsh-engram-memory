...
//
// Пакет самодостаточен: MCP-половина (engram и прокладка) лежит внутри пакета,
...
const GUIDANCE = [
  ...
  '- Проект записей подставляется сам — по рабочей области, где идёт разговор: называть его не нужно. Назови `project` только тогда, когда запись про другое место.',
...
/**
 * Где лежит engram. Первым идёт свой, из пакета: пакет самодостаточен и
 * не зависит от того, положил ли кто-то engram в приложение. Дальше — настройка,
...
export function apply(ctx, config = {}) {
  ...
  const saveCard = (args) =>
    new Promise((resolve, reject) => {
      ...
      if (binary === null) {
        reject(new Error('engram не найден'))
        return
      }
   ...
   * автосохранение: тот же engram, тот же каталог данных.
   */
  const runEngram = (args, { signal } = {}) =>
    new Promise((resolve) => {
      const binary = engramBinary(settings.engramPath)
      if (binary === null) {
        resolve({ ok: false, error: 'engram не найден' })
        return
      }
      const handle = store()
  ...
  const captureTurn = (state, turn = 'previous') => {
    ...
    if (binary === null) {
      ...
      warn('engram не найден — автосохранение выключено')
      return
    }
