...
// `ctx.systemPrompt` молча не существует — проверено живьём. `commands` и `llm`
// добавлены ради команды обобщения: первая объявляет её интерфейсу, вторая даёт
// модель харнесса без собственных ключей. Обе службы — базовая поставка.
export const inject = ['systemPrompt', 'commands', 'llm']
...
export function apply(ctx, config = {}) {
  ...
  const settings = {
    ...
    consolidateTimeoutMs: Number.isFinite(config.consolidateTimeoutMs) ? Number(config.consolidateTimeoutMs) : 90000,
  ...
  // со описанием, поэтому человеку не нужно помнить ни синтаксис, ни то, что
  // память вообще можно обобщать.
  if (settings.consolidate && typeof ctx.commands?.register === 'function') {
    try {
      const disposeCommand = ctx.commands.register({
        name: 'memory-consolidate',
        description: 'Обобщить память: свести сырые заметки проекта в карточки-выводы со ссылками на источники',
        ...
        handler: async (invocation) => {
          ...
          const route = routeFromAgent(invocation?.agent)
          if (route === undefined) {
            return { kind: 'error', text: 'Нет маршрута модели: обобщение идёт моделью харнесса, поэтому нужен хотя бы один состоявшийся ход в этой сессии.' }
          }
          try {
            return { kind: 'success', text: renderReport(await consolidateProject({ project, dryRun, route, signal: invocation?.signal })) }
          } catch (error) {
            return { kind: 'error', text: `Обобщение не удалось: ${error?.message ?? error}` }
          }
        }
      })
      ctx.on('dispose', () => disposeCommand?.())
    } catch (error) {
      warn(`команду обобщения объявить не удалось: ${error.message}`)
    }
  }
