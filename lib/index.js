...
export function apply(ctx, config = {}) {
  ...
  // Воркспейс, где сейчас идёт работа, запоминаем только для справки: список
  // проектов вкладка берёт из базы, а не из открытых окон — иначе невидимым
  // остаётся всё, что накопилось в закрытых проектах.
  let lastWorkspace = ''
  ...
  const modelOptions = async () => {
    ...
    const list = []
    try {
      const providers = await withTimeout(Promise.resolve(llmService.listProviders()), 3000)
      for (const entry of Array.isArray(providers) ? providers : []) {
        ...
        try {
          const models = await withTimeout(Promise.resolve(llmService.listModels(provider)), 3000)
