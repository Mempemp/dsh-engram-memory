...
import { createJob, panelStatistics, sendJson, stateHandler } from './panel.js'
...
export function apply(ctx, config = {}) {
  ...
  // Воркспейс и маршрут модели запоминаем по ходу работы: вкладке неоткуда взять
  // ни проект, ни модель — она открывается вне разговора.
  let lastWorkspace = ''
  let lastRoute
  let workspaceRoots = () => (lastWorkspace === '' ? [] : [lastWorkspace])

  /** Проект вкладки: воркспейс десктопа, иначе явная настройка. */
  const panelProject = () => {
    const db = store()
  ...
  /** Что показывает вкладка: статистика по базе плюс состояние последнего прохода. */
  ...
  const job = createJob(
    async (signal) => {
      ...
      if (lastRoute === undefined) throw new Error('нет маршрута модели: нужен хотя бы один состоявшийся ход в сессии')
      const result = await consolidateProject({ project, dryRun: false, route: lastRoute, signal })
