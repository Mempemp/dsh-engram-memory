...
export function renderRunReport(result) {
  ...
  const head = result.dryRun
    ? `Черновик по ${result.notes} заметкам: ${result.cards.length} карточк(и) — ничего не записано.`
    : `Сведено ${result.processed} из ${result.planned} заметок → ${result.saved.length} карточк(и) за ` +
      `${result.passes} ${plural(result.passes, 'проход', 'прохода', 'проходов')}.`
  const spent = result.usage ? ` Токенов: ${formatNumber(result.usage.totalTokens)}.` : ''
  const lines = [head + spent, `Проекты: ${result.plannedByProject.map((bucket) => `${bucket.project} — ${bucket.unprocessed}`).join(', ')}`]
  ...
  lines.push('Сырые заметки остались на месте.')
  return lines.join('\n')
}
