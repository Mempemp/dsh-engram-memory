...
check('без LocalAppData снимка нет', currentProjectFile({}) === null && writeCurrentProject('test-project', 'D:/Work/test-project', {}) === null && readCurrentProject({}) === null)
check('снимок пишется и читается', writeCurrentProject('test-project', 'D:/Work/test-project', env) !== null && readCurrentProject(env) === 'test-project')
check('пустое имя проекта не пишется', writeCurrentProject('   ', 'D:/Work/test-project', env) === null && readCurrentProject(env) === 'test-project')
writeFileSync(currentProjectFile(env), 'не json')
check('битый снимок — проекта нет', readCurrentProject(env) === null)
writeCurrentProject('test-project', 'D:/Work/test-project', env)

console.log('\n== прокладка читает снимок и конфиг ==')
const shim = await import('../mcp/bin/engram-mcp.js')
check('проект берётся из снимка', shim.projectFromState([currentProjectFile(env)]) === 'test-project', String(shim.projectFromState([currentProjectFile(env)])))
...
const injected = JSON.parse(shim.injectProject(writeCall, 'test-project'))
check('проект подставлен в запись', injected.params.arguments.project === 'test-project' && injected.params.arguments.title === 'x', shim.injectProject(writeCall, 'test-project'))
const namedCall = JSON.stringify({ jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: 'mem_save', arguments: { project: 'otherproject', content: 'y' } } })
check('названный проект не перебивается', JSON.parse(shim.injectProject(namedCall, 'test-project')).params.arguments.project === 'otherproject')
const searchCall = JSON.stringify({ jsonrpc: '2.0', id: 3, method: 'tools/call', params: { name: 'mem_search', arguments: { query: 'x' } } })
check('поиск проектом не сужается', shim.injectProject(searchCall, 'test-project') === searchCall)
const listCall = JSON.stringify({ jsonrpc: '2.0', id: 4, method: 'tools/list' })
check('служебные сообщения проходят как есть', shim.injectProject(listCall, 'test-project') === listCall)
check('строка не из JSON проходит как есть', shim.injectProject('не json', 'test-project') === 'не json')
